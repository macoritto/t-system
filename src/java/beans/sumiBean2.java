...
@ManagedBean
@ViewScoped
public class sumiBean2 implements Serializable{
    ...
    private Suministro suministroimpri;
    ...
    public sumiBean2() {
        ...
        this.suministroimpri = new Suministro();
    ...
    public List<OrdenDeCarga> getOrdenes() {
        OrdenDeCargaDao sumiDao = new OrdenDeCargaDaoImpl();
        this.ordenes = sumiDao.encontrarLiq(this.selectedCamion);
        return ordenes;
    }
    ...
    public void onRowSelectCamion2(AjaxBehaviorEvent event) {
        this.selectedSuministro.setCamion(this.selectedCamion);
        this.selectedSuministro.setChofer(this.selectedCamion.getChofer());
    }
    public void onRowSelectCamion(AjaxBehaviorEvent event) {
        String msg;
        CamionDao caDao = new CamionDaoImpl();
        this.camiones=caDao.findCamion(this.selectedCamion.getChapaCamion().toUpperCase());
    ...
    public void onRowSelectOrden2(AjaxBehaviorEvent event) {
        ...
        if (this.selectedOrdenDeCarga.getCamion() == null) {
            ...
            if (this.selectedOrdenDeCarga.getEstadoliq().equals("Pagado")) {
                ...
                if(this.selectedOrdenDeCarga.getIdOrdenDeCarga()==1){
                    ...
                    this.readonly=false;
                    this.selectedSuministro.setOrdenDeCarga(this.selectedOrdenDeCarga);
                    msg = "Se selecciono la orden para suministros sin Flete";
                    FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, msg, "");
                    FacesContext.getCurrentInstance().addMessage(null, message);
                }else{
   ...
   public String cargarArray(ActionEvent event) {
    ...
    public void btnCreateComb(ActionEvent actionEvent) {
        ...
        this.suministroimpri=this.selectedSuministro;
    ...
    public void exportarPDF2(ActionEvent actionEvent) throws JRException, IOException, ClassNotFoundException, SQLException {
        codvale = this.suministroimpri.getIdSuministro();
        if (this.suministroimpri.getTipoSuministro().getIdTipo() == 0 || this.suministroimpri.getTipoSuministro().getIdTipo() == 3) {
            File jasper = new File(FacesContext.getCurrentInstance().getExternalContext().getRealPath("/reportes/viatico.jasper"));
            String letras = "";
            letras = Numero_a_Letra.cantidadConLetra(this.suministroimpri.getMonto().toString());
            ...
            HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
            response.addHeader("Content-disposition", "attachment; filename=suministro N° " + this.suministroimpri.getIdSuministro() + ".pdf");
            ServletOutputStream stream = response.getOutputStream();
            JasperExportManager.exportReportToPdfStream(jasperPrint, stream);
            stream.flush();
            stream.close();
            ...
            if (this.suministroimpri.getTipoSuministro().getIdTipo() == 1) {
